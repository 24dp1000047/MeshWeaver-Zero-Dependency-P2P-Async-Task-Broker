"""
heartbeat.py — Sahil's Heartbeat-based failure detection.

Restored from main-branch heartbeat.py (was misplaced at project root).
This module provides:
  - NodeState: enum for ALIVE / SUSPECTED / OFFLINE.
  - HeartbeatManager: processes incoming heartbeats, tracks last-seen
    timestamps, transitions node states, and fires offline callbacks.
"""
        ...
        for peer, ts in list(self.last_seen.items()):
            ...
            if delta >= self.offline_t:
                if st != NodeState.OFFLINE:
                    ...
                    logger.error(
                        f"Node {peer} unreachable. Marking OFFLINE after {round(delta, 1)}s"
                    )
                ...
                if st == NodeState.ALIVE:
                    ...
                    logger.warning(
                        f"Missed heartbeats from {peer}. State changed to SUSPECTED"
                    )
